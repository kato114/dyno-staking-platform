...
const fetchFetchPublicDataOld = async ({ pids, chainId }): Promise<[SerializedFarm[], number, number]> => {
  const [poolLength, [cakePerBlockRaw]] = await Promise.all([
    ...
    multicall(masterchefABI, [
      {
        // BSC only
        address: getMasterChefAddress(ChainId.DYNO),
        name: 'cakePerBlock',
        params: [true],
      },
    ]),
  ...
  const rewardPerBlock = getBalanceAmount(new BigNumber(cakePerBlockRaw))
  ...
  return [farmsWithPrices, poolLengthAsBigNumber.toNumber(), rewardPerBlock.toNumber()]
}

const fetchFarmPublicDataPkg = async ({ pids, chainId, chain }): Promise<[SerializedFarm[], number]> => {
  const farmsConfig = await getFarmConfig(chainId)
  const farmsCanFetch = farmsConfig.filter((farmConfig) => pids.includes(farmConfig.pid))
  const priceHelperLpsConfig = getFarmsPriceHelperLpFiles(chainId)
  const { farmsWithPrice, poolLength } = await farmFetcher.fetchFarms({
    chainId,
...
>(
  ...
  async ({ account, pids, proxyAddress, chainId }, config) => {
    const poolLength = config.getState().farms.poolLength ?? (await fetchMasterChefFarmPoolLength(ChainId.DYNO))
    const farmsConfig = await getFarmConfig(chainId)
    const farmsCanFetch = farmsConfig.filter(
