...
import dynostaking from 'config/abi/dynostaking.json'
...
export const fetchFarmUserEarnings = async (account: string, farmsToFetch: SerializedFarmConfig[], chainId: number) => {
  const multiCallChainId = ChainId.DYNO
  ...
  const rawEarnings = await multicallv2({ abi: dynostaking, calls, chainId: multiCallChainId })
