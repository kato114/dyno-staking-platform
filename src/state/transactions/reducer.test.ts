...
describe('transaction reducer', () => {
  ...
  describe('addTransaction', () => {
    it('adds the transaction', () => {
      ...
      expect(txs[ChainId.DYNO]).toBeTruthy()
      expect(txs[ChainId.DYNO]?.['0x0']).toBeTruthy()
      const tx = txs[ChainId.DYNO]?.['0x0']
  ...
  describe('finalizeTransaction', () => {
    ...
    it('sets receipt', () => {
      ...
      const tx = store.getState()[ChainId.DYNO]?.['0x0']
      expect(tx?.summary).toEqual('hello world')
  ...
  describe('clearAllChainTransactions', () => {
    it('removes all transactions for the chain', () => {
      ...
      expect(Object.keys(store.getState())).toEqual([String(ChainId.DYNO)])
      expect(Object.keys(store.getState()[ChainId.DYNO] ?? {})).toEqual(['0x0'])
      store.dispatch(clearAllChainTransactions({ chainId: ChainId.DYNO }))
      expect(Object.keys(store.getState())).toHaveLength(2)
      expect(Object.keys(store.getState())).toEqual([String(ChainId.DYNO)])
      expect(Object.keys(store.getState()[ChainId.DYNO] ?? {})).toEqual([])
    })
  })
