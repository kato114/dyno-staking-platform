import { ChainId, Currency, CurrencyAmount, JSBI, Pair, Price, Token, WNATIVE, WDND } from '@pancakeswap/sdk'
import { FAST_INTERVAL } from 'config/constants'
import { USDT, DUSD } from '@pancakeswap/tokens'
...
export default function useBUSDPrice(currency?: Currency): Price<Currency, Currency> | undefined {
  ...
  const stable = DUSD[chainId] || USDT[chainId]
...
export const useCakeBusdPrice = ({ forceMainnet } = { forceMainnet: false }): Price<Currency, Currency> | undefined => {
  const { chainId } = useActiveWeb3React()
  return usePriceByPairs(DUSD[chainId], WDND[chainId])
}

// @Note: only fetch from one pair
export const useDNDDusdPrice = ({ forceMainnet } = { forceMainnet: false }): Price<Currency, Currency> | undefined => {
  const { chainId } = useActiveWeb3React()
  return usePriceByPairs(DUSD[chainId], WDND[chainId])
}
