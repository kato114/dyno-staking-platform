...
export function getBlockExploreLink(
  ...
  const chainId = chainIdOverride || ChainId.DYNO
  const chain = chains.find((c) => c.id === chainId)
  if (!chain) return dyno.blockExplorers.default.url
  switch (type) {
    case 'transaction': {
...
export function getBlockExploreName(chainIdOverride?: number) {
  ...
  return chain?.blockExplorers?.default.name || dyno.blockExplorers.default.name
}

export function getBscScanLinkForNft(collectionAddress: string, tokenId: string): string {
  return `${dyno.blockExplorers.default.url}/token/${collectionAddress}?a=${tokenId}`
}
