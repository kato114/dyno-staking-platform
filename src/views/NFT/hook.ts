...
import { useDNDDusdPrice } from 'hooks/useBUSDPrice'
...
export const usePriceDNDBusd = ({ forceMainnet } = { forceMainnet: false }): BigNumber => {
...
export const useNFTContract = ({ id } = { id: 1 }): any => {
  ...
  const allowance = useSingleCallResult(wDNDContract, 'allowance', [account, NftContractInfo.address]).result
