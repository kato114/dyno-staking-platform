...
const WalletInfo: React.FC<WalletInfoProps> = ({ hasLowNativeBalance, onDismiss }) => {
  ...
  const isBSC = chainId === ChainId.DYNO
  const bnbBalance = useBalance({ addressOrName: account, chainId: ChainId.DYNO })
      ...
      <Box mb="24px">
        <Flex justifyContent="space-between" alignItems="center" mb="8px">
          <Flex bg={COLORS.BNB} borderRadius="16px" pl="4px" pr="8px" py="2px">
            <ChainLogo chainId={ChainId.DYNO} />
            <Text color="white" ml="4px">
              BNB Smart Chain
            </Text>
          </Flex>
          <LinkExternal href={getBlockExploreLink(account, 'address', ChainId.DYNO)}>
            {getBlockExploreName(ChainId.DYNO)}
          </LinkExternal>
        </Flex>
        <Flex alignItems="center" justifyContent="space-between">
